...
/* Shared Card Tools USB picker UI/controller. */
...
static void RenderPicker(const char *title, const char *card_role,
                         const int *card_port, const MciUsbPickerList *list,
                         int row, int first)
{
    char body[1024];
    char path_line[104];
    ...
    snprintf(path_line, sizeof(path_line), "%s%.*s",
             strlen(list->path) > 72u ? "..." : "", 72, tail);
    if (card_port != NULL) {
        used = snprintf(body, sizeof(body),
                        "USB  %s\n%s  mc%d\n\n",
                        path_line,
                        card_role != NULL ? card_role : "CARD",
                        *card_port);
    } else {
        used = snprintf(body, sizeof(body), "USB  %s\n\n", path_line);
    }
    ...
    if (list->truncated && (unsigned int)used < sizeof(body) - 48u)
        snprintf(body + used, sizeof(body) - (unsigned int)used,
                 "\n[List truncated to %d entries]",
                 MCI_USB_PICKER_MAX_ENTRIES);

    MciGuiRenderMessage(title != NULL ? title : "USB FILE PICKER", body,
                        card_port != NULL
                            ? "UP/DOWN Move  X Open/Choose  CIRCLE Parent/Cancel  TRIANGLE USB  L1 mc0  R1 mc1"
                            : "UP/DOWN Move  X Open/Choose  CIRCLE Parent/Cancel  TRIANGLE USB",
                        MCI_GUI_TONE_INFO);
}
