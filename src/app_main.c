...
/*
 * PS2 Memory Card Inspector 0.4.1 release controller.
 *
 ...
 * collapse to one probe. The resulting bound bytes are retained only for the
 * immediately following transaction so no security reboot is needed after the
 * USB recovery journal becomes active.
 */
...
int main(int argc, char *argv[])
{
    ...
    if (MciGuiInit() < 0) {
        scr_clear();
        scr_printf("PS2 Memory Card Inspector 0.4.1 Drebin\n\n");
