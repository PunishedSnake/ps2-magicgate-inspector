 ...
 * reason. ROMVER remains authoritative for the OSDSYS folder because that is
 * the name the running boot ROM actually searches; MechaCon state tells us
 * whether the console is likely to accept a less region-specific KELF policy.
 */
...
static int ProbeMecha(MciConsoleProfile *profile)
{
     ...
     * MechaCon minor revision (for example 5.8/5.9). Do not label this
     * "MechaPwn detected": real DEX hardware and another compatible patch can
     * present the same effective state. The installer cares about capability. */
...
int MciConsoleProfileProbe(MciConsoleProfile *profile)
{
    ...
    /* Do not compare the raw SCMD 0x36 payload to ROMVER as an installation
     * blocker until that structure has been validated across Dragon/Deckard
     * retail and modified hardware. Deckard MechaPwn already changes the
     * active ROMVER; older Dragon units keep their ROM region but expose the
     * DEX-like MechaCon revision signal. Those two facts are sufficient for a
