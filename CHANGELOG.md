...
- dev13 corrects the Inspector's SECR port numbering at the SECRSIF RPC boundary. The reference FreeMcBoot Installer calls `SecrDownloadFile(2 + port, slot, ...)`, because CardAuth consumes the physical SIO2 memory-card channels 2/3 directly. Inspector had passed logical libmc ports 0/1, which targeted controller channels and produced missing-ACK failures. HEADER, GET_KBIT and GET_KC now map logical 0/1 to physical 2/3 while normal libmc access remains unchanged.
